...
// Find all scripts with a "datacjssrc" attribute.
CJS.downloadScripts = function() {
	var len = CJS.aScripts.length;
	for ( var i = 0; i < len; i++ ) {
		var script = CJS.aScripts[i];
		var src = CJS.getAttribute(script, "datacjssrc") || CJS.getAttribute(script, "cjssrc"); // backward compatible to earlier "cjssrc" attribute
		if ( src ) {
